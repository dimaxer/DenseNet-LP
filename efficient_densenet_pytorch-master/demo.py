...
def train_epoch(model, train_batch_list, optimizer, epoch, n_epochs, print_freq=1):
    ...
    for batch_idx , batch in enumerate(train_batch_list):
        ...
        labels = np.array_split(PreProcess.trueLables, PreProcess.batch_size_divider)
        torchLabels = torch.cuda.LongTensor(labels[batch_idx])
...
def train(model, test_batch_list, valid_set, train_batch_list, save, n_epochs=300,
...
def demo(data, save, depth=100, growth_rate=12, efficient=True, valid_size=5000,
    ...
    train_set = PreProcess.load_data_from_file(PreProcess.training_data_file_name)
    train_batch_list = np.array_split(train_set, PreProcess.batch_size_divider)

    # create test data
    test_set = PreProcess.load_data_from_file(PreProcess.test_data_file_name)
    test_batch_list = np.array_split(test_set, PreProcess.batch_size_divider)
    ...
    # Train the model
    train(model=model, train_batch_list=train_batch_list, valid_set=valid_set, test_batch_list=test_batch_list, save=save,
